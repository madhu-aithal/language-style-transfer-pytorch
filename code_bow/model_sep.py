...
class Model(nn.Module):
    ...
    def generate_x(self, hidden_vec, true_outputs, criterion, teacher_forcing=False):
        ...
        gen_input = torch.tensor([self.GO_token], device=self.device)
        # gen_input = torch.tensor(sp.EncodeAsIds("<go>"), device=self.device)
        ...
        for i in range(input_length):            
            ...
            if teacher_forcing == True:
                ...
                # gen_input = torch.argmax(self.softmax(gen_output), dim=1)
    ...
    def train_one_batch(self, training_data, target, sentiment):
        ...
        # criterion = nn.CrossEntropyLoss(ignore_index=sp.EncodeAsIds("<pad>")[0], reduction='mean', size_average=True)
    ...
    def forward(self, args, batches0, batches1, dev_batches0, dev_batches1, vocab, no_of_epochs, epoch, writer, time, save_epochs_flag=False, 
        ...
        for batch0, batch1 in zip(batches0, batches1):
            ...
            self.discrim1_optim.zero_grad()
            self.discrim2_optim.zero_grad()
            ...
            if self.args.pretrain_flag == True:
                if self.args.autoencoder_pretrain_flag == True:
                    ...
                    self.discrim1_optim.step()
                    self.discrim2_optim.step()
            else:                        
                if flag == True:
        ...
        if self.args.dev:
            ...
            with torch.no_grad():
                ...
                random.shuffle(dev_batches0)
                random.shuffle(dev_batches1)

                for dev_batch0, dev_batch1 in zip(dev_batches0, dev_batches1):
                    loss_adv1, loss_adv2, loss_enc_gen, loss_reconstruction, disc_batch_dev_acc = self.train_util(dev_batch0, dev_batch1, epoch, writer)
